#pragma once
#ifndef CPP_CURL_EASY_H
#define CPP_CURL_EASY_H 1
...
public:
     ...
     * */
    static void add_easy_extant(unsigned int num=1) noexcept;
    /*
     * @brief: 减少现存数量
     * @param: num: 减少的数量
     * @attention: 仅在启用自动管理时有意义
     * */
    static void sub_easy_extant(unsigned int num=1) noexcept;
    // @brief: 获取现存数量
     ...
     * */
    template <typename T> bool setOption(CURLoption&& option, T&& value) NOEXCEPT
    {
        _error = curl_easy_setopt(_curl, std::forward<CURLoption>(option), std::forward<T>(value));
        if (_error != CURLE_OK) {
    ...
    {
        return setOption(std::forward<CURLoption>(option), std::forward<T>(value))
     ...
     * */
    template <typename T> bool getinfo(CURLINFO info, T& value) NOEXCEPT
    {
     ...
     * @param: n: 数据长度
     * */
    template <typename T> bool recv(T* buffer, size_t* n) NOEXCEPT
    {
     ...
     * @param: n: 数据长度
     * */
    template <typename T> bool send(T* buffer, size_t* n) NOEXCEPT
    {
