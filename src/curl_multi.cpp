...
void curl_multi::auto_manage(bool flag) NOEXCEPT { _auto_manage = flag; }
void curl_multi::add_multi_extant(unsigned int num) noexcept { _multi_extant += num; }
void curl_multi::sub_multi_extant(unsigned int num) noexcept { _multi_extant -= num; }
size_t curl_multi::extant() noexcept { return _multi_extant; }
...
bool curl_multi::addHandle(const curl_easy& easy) NOEXCEPT
{
    if (_curlm == nullptr)
        return false;
    _easy_handles.emplace_back(const_cast<curl_easy*>(&easy));
...
bool curl_multi::rmHandle(const curl_easy& easy) NOEXCEPT
{
    if (_curlm == nullptr)
        return false;
...
{
    _msg = curl_multi_info_read(_curlm, msgs_in_queue);
...
bool curl_multi::fdset() NOEXCEPT
{
    ...
    FD_ZERO(&_multi_fd[fd_write]);
    FD_ZERO(&_multi_fd[fd_except]);
    _error = curl_multi_fdset(
        _curlm, &_multi_fd[fd_read], &_multi_fd[fd_write], &_multi_fd[fd_except], &_max_fd);
    if (_error != CURLM_OK) {
        _error_vec.emplace_back(_error);
#ifdef CURL_ERROR_ON
        THROW_CURL_ERROR(_error);
...
{
    int _select = ::select(
        _max_fd + 1, &_multi_fd[fd_read], &_multi_fd[fd_write], &_multi_fd[fd_except], &_timeout);
#ifdef CURL_ERROR_ON
    if (_select == -1)
...
bool curl_multi::poll(
    struct curl_waitfd extra_fds[], uint extra_nfds, int timeout_ms, int& numfds) NOEXCEPT
{
...
CURLM* curl_multi::getHandle() const noexcept { return _curlm; }
CURLMcode curl_multi::getError() const noexcept { return _error; }
const char* curl_multi::getErrorText() const noexcept { return curl_multi_strerror(_error); }
int curl_multi::getEasyExtant() const noexcept { return _easy_extant; }
const std::vector<CURLMcode>& curl_multi::getErrorVec() const NOEXCEPT { return _error_vec; }
bool curl_multi::isRunning() const noexcept { return _is_running; }
std::vector<curl_easy*> curl_multi::getEasyHandles() const NOEXCEPT { return _easy_handles; }
int curl_multi::getMaxFd() const noexcept { return _max_fd; }
std::array<fd_set, 3> curl_multi::getMultiFd() const noexcept { return _multi_fd; }
struct timeval curl_multi::getTimeout() const noexcept { return _timeout; }
    ...
    : _msg(msg)
{
    if (msg == nullptr)
        return;
    for (auto* easy : multi.getEasyHandles()) {
        if (easy->getHandle() == _msg->easy_handle) {
            _easy = easy;
            break;
        }
    }
...
bool curl_msg::init(CURLMsg* msg, curl_multi& multi) noexcept
{
    curl_msg tmp(msg, multi);
    if (!tmp)
        return false;
    _msg = std::exchange(tmp._msg, nullptr);
    _easy = std::exchange(tmp._easy, nullptr);
    return true;
}
CURLMsg* curl_msg::getMsg() const noexcept { return _msg; }
curl_easy* curl_msg::getEasy() const noexcept { return _easy; }
} // namespace web
